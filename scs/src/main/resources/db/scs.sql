...
CREATE TABLE `user` (
                        `id` bigint(20) NOT NULL COMMENT '主键',
                        `username` varchar(50) COLLATE utf8_bin DEFAULT NULL COMMENT '账号',
...
-- 2. category -> t_menu_category
ALTER TABLE `category` RENAME TO `t_menu_category`;

-- 3. dish -> t_menu_dish
ALTER TABLE `dish` RENAME TO `t_menu_dish`;

-- 4. dish_flavor -> t_menu_flavor
ALTER TABLE `dish_flavor` RENAME TO `t_menu_flavor`;

-- 5. employee -> t_sys_employee
ALTER TABLE `employee` RENAME TO `t_sys_employee`;

-- 6. orders -> t_order
ALTER TABLE `orders` RENAME TO `t_order`;

-- 7. order_detail -> t_order_item
ALTER TABLE `order_detail` RENAME TO `t_order_item`;

-- 8. setmeal -> t_menu_combo
ALTER TABLE `setmeal` RENAME TO `t_menu_combo`;

-- 9. setmeal_dish -> t_menu_combo_dish
ALTER TABLE `setmeal_dish` RENAME TO `t_menu_combo_dish`;

-- 10. shopping_cart -> t_user_cart
ALTER TABLE `shopping_cart` RENAME TO `t_user_cart`;

-- 11. user -> t_user
ALTER TABLE `user` RENAME TO `t_user`;

-- 插入测试用户数据（账号: admin, 密码: 123456）
INSERT INTO `t_user` (`id`, `username`, `password`, `name`, `phone`, `status`) 
VALUES (1, 'admin', '123456', '管理员', '13800138000', 1);
